...
SharedQueue* init_shm(int& shmid) {
    ...
    if (shmid < 0 && errno == EPERM) {
        std::cerr << "Warning: cannot allocate huge pages (EPERM). Falling back to normal pages.\n";
...
int main() {
    ...
    cleanup(shmid, queue,
            sem_init, sem_filled, sem_empty,
            sem_consumed, sem_shutdown);
